...
#include <raylib.h>
#include <raymath.h>
Texture2D plasmaPNG, background;
// --- UTILS ---
namespace Utils {
...
public:
    ...
    Color AsteroidColor = BLACK;
    Color fillColor = RED; 
    DrawPoly(transform.position, 3, GetRadius() + 3, transform.rotation, AsteroidColor);

    DrawPoly(transform.position, 3, GetRadius(), transform.rotation, fillColor);
}
};
...
public:
	...
	if (type == WeaponType::BULLET) {
		...
		if (plasmaPNG.id != 0) {
        float scale = 0.1f; // skala do dopasowania rozmiaru
        Vector2 origin = { plasmaPNG.width / 2.f, plasmaPNG.height / 2.f }; // rysuj względem środka
        DrawTextureEx(plasmaPNG, transform.position, 0.0f, scale, WHITE);
		}
		else DrawCircleV(transform.position, 8.f, BLUE);
	}
}
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("C:/Users/czare/Documents/projekty/lab1/lab1/source/spaceship1.png");
...
public:
	...
	void Run() {
		...
		float shotTimer = 0.f;
		 plasmaPNG= LoadTexture("C:/Users/czare/Documents/projekty/lab1/lab1/source/plasma.png");
		background= LoadTexture("C:/Users/czare/Documents/projekty/lab1/lab1/source/bg.png");
		while (!WindowShouldClose()) {
			float scaleX = (float)GetScreenWidth() / background.width;
            float scaleY = (float)GetScreenHeight() / background.height;
Vector2 pos = {0, 0};

DrawTextureEx(background, pos, 0.0f, fmaxf(scaleX, scaleY), WHITE);
